...
s2d_scene_create(struct s2d_device* dev, struct s2d_scene** out_scn)
{
  ...
  scn = (struct s2d_scene*)MEM_CALLOC
    (dev->allocator, 1, sizeof(struct s2d_scene));
  if(!scn) {
    res = RES_MEM_ERR;
    goto error;
  }
...
res_T
s2d_scene_ref_get(struct s2d_scene* scn)
{
...
res_T
s2d_scene_ref_put(struct s2d_scene* scn)
{
  if(!scn) return RES_BAD_ARG;
