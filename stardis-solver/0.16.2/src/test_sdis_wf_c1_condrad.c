 ...
 *
 * 9 probes in solid region x ∈ [-0.8, 0.8] (y=0, z=0).
 * Dual validation A+B.
 *
...
c1_analytic(double u)
{
   ...
   * Called from p0_run_probe_sweep with x ∈ [0, 1] representing
   * the probe parameter; we map it to physical x ∈ [-0.8, 0.8] */
  double x = -0.8 + 1.6 * u;
...
main(int argc, char** argv)
{
  ...
  /* We probe x ∈ [-0.8, 0.8], passing through c1_analytic()
   * which maps the normalised parameter u ∈ [0,1] to physical x.
